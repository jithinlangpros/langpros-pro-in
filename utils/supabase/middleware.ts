...
export async function updateSession(request: NextRequest) {
  ...
  const isPublicRoute = request.nextUrl.pathname === '/' || request.nextUrl.pathname === '/unauthorized'
  ...
  if (user && !isPublicRoute) {
    ...
    // Restrict access
    if (path.startsWith('/project-manager') && role !== 'pm') {
      return NextResponse.redirect(new URL('/unauthorized', request.url))
    }
    if (path.startsWith('/inventory-manager') && role !== 'inv') {
      return NextResponse.redirect(new URL('/unauthorized', request.url))
    }
